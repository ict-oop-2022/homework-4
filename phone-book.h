...
struct  comp1_t{
    bool operator() (const user_info_t& user1, const user_info_t& user2) const{
        if (user1.user.name != user2.user.name){
            //лексикографическому возрастанию имени,
            return user1.user.name < user2.user.name;
        }
        if (user1.total_call_duration_s != user2.total_call_duration_s){
            //по убыванию суммарной длительности звонков на этот номер,
            return user1.total_call_duration_s > user2.total_call_duration_s;
        }
        //по лексикографическому возрастанию номера
        return user1.user.number < user2.user.number;
    }
};
...
class phone_book_t{
public:
    std::set<user_info_t, comp1_t> book;
    std::unordered_map<std::string, user_info_t> users;
    std::unordered_map<std::string, std::set<user_info_t, comp2_t>> book_numbers;
    std::vector<call_t> calls_to_num;
...
public:
     ...
     */
    std::vector<user_info_t> search_users_by_number(const std::string &number_prefix, size_t count) ;
     ...
     * @return vector of search result, sorted by rules above
     */
    std::vector<user_info_t> search_users_by_name(const std::string &name_prefix, size_t count);
